...
class EntityGlobalPassportSDK:
    """Thin public facade over the v3.4 registries. It does not create authority or regulatory status."""
    def __init__(self,profile_registry,global_passports,continuous_ingestion):
    ...
    def compose_profiles(self,*names_or_refs:str)->dict:
        refs=[]
        if not any(str(x).startswith("entity-profile:global@") or str(x).lower()=="global" for x in names_or_refs): refs.append(PROFILE_ALIASES["global"])
    ...
    def register_file(self,path,controller_entity_id:str,*profiles:str,logical_path:str|None=None,previous_object_id:str|None=None,version:str="1.0")->dict:
    ...
    def capability_status()->dict:
        return {"schema":"entity-v3-global-passport-sdk-status-v1","sdk_does_not_create_authority":True,
                ...
                "continuous_provenance_supported":True,"built_in_profile_aliases":sorted(PROFILE_ALIASES)}
