...
OUT = ROOT / "runtime" / "trl9" / "BECP_0.2.1_TRL9_20260912" / "PRODUCTION_MCP_RESULTS.json"
URL = "https://127.0.0.1:8767/mcp"
...
def issue_token() -> str:
    ...
    return SignedTokenService(secret).issue({
        "sub": "becp-0.2.1-trl9-production-mcp",
        "subject": "BECP 0.2.1 TRL9 Production MCP",
    ...
    try:
        async with streamable_http_client(URL, http_client=client) as (read_stream, write_stream):
            async with ClientSession(read_stream, write_stream) as session:
                ...
                boot = extract(await session.call_tool("becp_session_bootstrap", {"metadata": {"qualification": "0.2.1-trl9-production-mcp"}}))
